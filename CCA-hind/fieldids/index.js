    ...
    if (desiredDocument) {
      ...
      for (let i = 0; i < 8; i++) {
        for (const field of desiredDocument.fields[i].children) {
          ...
          englishMap[fieldTranslationsEn.name] = field.fieldId;
          hindiMap[fieldTranslationsHi.name] = field.fieldId;

          // Check if field has valueOptions
          if (field.valueOptions && field.valueOptions.length > 0) {
            valueOptionsMap[field.translations.hi.name] = field.valueOptions;
          }

          // Check if field has TYPE
          if (field.type) {
            typeMap[fieldTranslationsHi.name] = field.type;
          }
        }
      }
      ...
      fs.writeFile('field_type.json', typeOutputData, 'utf8', (err) => {
        if (err) {
          console.errorin('Error writing TYPE to file:', err);
        } else {
